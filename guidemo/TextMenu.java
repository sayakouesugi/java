...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; // controls whether the text is left aligned.
	...
	public TextMenu(DrawPanel owner) {
		...
		// Adding Line Height functionality
		...
		lineHeight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String s = JOptionPane.showInputDialog(panel, "What line height multiplier do you want to use?", currentSize);
				if (s != null && s.trim().length() > 0) {
					try {
						double newSize = Double.parseDouble(s.trim());
						panel.getTextItem().setLineHeightMultiplier(newSize);
						panel.repaint();
					}
					catch (Exception e) {
	...
	private JMenu makeJustifyMenu() {
		...
		ActionListener setRightAction = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(TextItem.RIGHT);
				panel.repaint();
			}
		};
		right.addActionListener(setRightAction);
		group.add(right);
		menu.add(right);

		JRadioButtonMenuItem center = new JRadioButtonMenuItem("Center");
		ActionListener setCenterAction = new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				panel.getTextItem().setJustify(TextItem.CENTER);
				panel.repaint();
			}
		};
		center.addActionListener(setCenterAction);
		group.add(center);
		menu.add(center);
